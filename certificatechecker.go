//	is root in?
//	all chains - use golang to test all? maybe graph like Rob's crt.sh
...
func CheckCertificate(address string) CertResult {
	...
	for _, cert := range conn.ConnectionState().PeerCertificates {
		...
		if certchecker.CertificateOwner[hex.EncodeToString(sha256HashCert.Sum(nil))] != "" {
			thisCertificate.CertificateOwner = certchecker.CertificateOwner[hex.EncodeToString(sha256HashCert.Sum(nil))]
		}
		i++
	}
	...
	if thisCertificate.CertSubjectCN == thisCertificate.CertIssuerCN || strings.Contains(thisCertificate.CertIssuerCN, thisCertificate.CertSubjectCN) {
		...
		for _, thisOID := range PolicyOIDArray {
			_, evOID := certchecker.EVIssuers[thisOID]
			if evOID {
